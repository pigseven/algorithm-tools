	...
	def obtain_object_id_and_url(request_upload_file_url, expires_in = None):
		...
		while True:
			...
			if retry < 3:
				...
				raise RuntimeError('ERROR: reach max request time = 3, cannot obtain object_id & upload_url')
	...
	def upload_file(put_file_url, file_path, content_type):
		...
		while True:
			...
			if retry < 3:
				...
				raise RuntimeError('ERROR: reach max request time = 3, cannot upload file')
