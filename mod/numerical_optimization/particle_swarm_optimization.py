...
class PSO(object):
	...
	def _init_particle_locs(self):
		...
		for i in range(self.dim):
			...
			locs = np.random.random(self.particles_num) * (bounds[1] - bounds[0]) + bounds[0]
			
			if i == 0:
				self.particle_locs = locs.reshape(-1, 1)
			else:
				self.particle_locs = np.hstack((self.particle_locs, locs.reshape(-1, 1)))
	...
	def _init_particle_vels(self):
		...
		for i in range(self.dim):
			...
			vels = np.random.random(self.particles_num) * (bounds[1] - bounds[0]) + bounds[0]
			
			if i == 0:
				self.particle_vels = vels.reshape(-1, 1)
			else:
				self.particle_vels = np.hstack((self.particle_vels, vels.reshape(-1, 1)))
	...
	def current_particles_locs_and_values(particle_locs, obj_func):
		...
		current_particles_info['obj_value'] = current_particles_info.apply(obj_func, axis = 1)
	...
	def global_best_solutions_and_value(particles_info, dim):
		...
		g_best_solution = np.array(particles_info.loc[current_g_best_idx, range(dim)])  # 此刻全局最优解
	...
	def optimization(self, w_decay = None, show_loss = False):
		...
		# 计算当前所有颗粒位置对应函数值
		current_particles_info = self.current_particles_locs_and_values(self.particle_locs, self.obj_func)
		
		# 初始化全局最优解和值
		g_best_solution, g_best_value = self.global_best_solutions_and_value(current_particles_info, self.dim)
		...
		for step in range(self.iter_num):
			...
			for i in range(len(self.init_vel_bounds)):
				self.particle_vels[:, i][self.particle_vels[:, i] > self.init_vel_bounds[i][1]] = self.init_vel_bounds[i][1]
				self.particle_vels[:, i][self.particle_vels[:, i] < self.init_vel_bounds[i][0]] = self.init_vel_bounds[i][0]
			
			# 位置更新
			self.particle_locs += self.particle_vels
			
			# 计算当前所有颗粒位置函数值, 并更新全局最优解记录
			current_particles_info_tmp = self.current_particles_locs_and_values(self.particle_locs, self.obj_func)
			g_best_solution_tmp, g_best_value_tmp = self.global_best_solutions_and_value(current_particles_info_tmp, self.dim)
			...
			for row in range(self.particles_num):
				if current_particles_info_tmp.loc[row, 'obj_value'] < current_particles_info.loc[row, 'obj_value']:
					current_particles_info.loc[row, :] = current_particles_info_tmp.loc[row, :]
			
			loss.append(g_best_value)
			particles_locus_record.append(self.particle_locs.copy())
			
			print('iter: {}, loss: {:.6f}, current best solution: {}'.format(step, g_best_value, g_best_solution))
		...
		if show_loss:
			...
			if self.dim == 1:
				...
				for i in range(self.particles_num):
					plt.plot(
						[r[i, 0] for r in particles_locus_record], [r[i, 1] for r in particles_locus_record],
						linewidth = 0.5,
						alpha = 0.4
					)
				plt.scatter(g_best_solution[0], g_best_solution[1], c = 'k', s = 80, alpha = 1.0, label = 'optimal')
