...
from mod.data_process.replace_singulars import replace_singulars2nans
...
if __name__ == '__main__':
	...
	# %% 异常值处理.
	# 一般认为各污染物浓度不可能为0.0, 如果记录中存在0值则认为异常, 使用np.nan替代,
	# 然后在数据填补过程中进行填充.
	cols2check = ['pm25', 'pm10', 'so2', 'no2', 'o3', 'co', 'aqi']
	singular_value = 0.0
	data = replace_singulars2nans(data_raw, cols2check, singular_value)
	...
	dts = DataTemporalSerialization(data, start_stp, end_stp, stp_step, cols2serialze)
